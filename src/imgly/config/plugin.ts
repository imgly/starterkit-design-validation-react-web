/**
 * Design Validation Editor Plugin - Complete Design Editing Configuration for CE.SDK
 *
 * This plugin provides a production-ready design validation editor configuration with
 * templates, elements, asset libraries, and comprehensive editing capabilities.
 *
 * @example Basic usage
 * ```typescript
 * import CreativeEditorSDK from '@cesdk/cesdk-js';
 * import { DesignValidationEditorConfig } from './plugin';
 *
...
/**
 * Design Validation Editor configuration plugin.
 *
 ...
 * @public
 */
export class DesignValidationEditorConfig implements EditorPlugin {
  /**
   * Unique identifier for this plugin.
   * Used to identify the plugin in the CE.SDK plugin registry.
   */
  name = 'cesdk-design-validation-editor';
  ...
  /**
   * Initialize the design validation editor configuration.
   *
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // This ensures a clean slate when applying the design validation editor config
      cesdk.resetEditor();
      // #endregion

      // #region Role and Theme Configuration
      // Set user role and UI theme via runtime APIs
